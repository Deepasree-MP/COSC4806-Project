    ...
    <div class="row g-4">
      <?php foreach ($ratings as $row): ?>
        ...
        <div class="col-md-4 col-lg-3">
          <div tabindex="0"
               class="card rating-card h-100 shadow-lg"
               onclick="window.location='/movie/search?title=<?= urlencode($row['movie_title']) ?>'"
               onkeypress="if(event.key==='Enter'){window.location='/movie/search?title=<?= urlencode($row['movie_title']) ?>'}">
            <?php if ($img): ?>
